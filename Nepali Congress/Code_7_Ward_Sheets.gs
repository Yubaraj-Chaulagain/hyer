...
function wardSheetName(data) {
  const no = normalizeWardNo(data['Ward No.'] || data.WardNo || data.wardNo || data.ward || data['Ward Number']);
...
function wardSheetNameFromRecord(data) {
  if (data.sheet && Object.values(WARD_SHEETS).indexOf(String(data.sheet)) !== -1) return String(data.sheet);
...
function addWard(data) {
  ...
  appendObject(sh, row);
  return {success:true, message:'Ward record submitted successfully',
    wardNo:normalizeWardNo(data['Ward No.'] || data.WardNo || data.wardNo || data.ward),
    sheet:sheetName, cn:cn, row:sh.getLastRow()};
}

function updateWard(data) {
  const sheetName = wardSheetNameFromRecord(data);
  const row = findRow(sheetName, data, ['row','C.N','Name']);
  updateObjectAtRow(sheet(sheetName), row, data,
    ['action','token','username','adminAction','row','sheet']);
  ...
  Object.keys(WARD_SHEETS).forEach(no => {
    ...
    try {
      rows = rows.concat(getSheetRows(sheetName).map(r => {
        r.sheet = sheetName;
        r['Ward No.'] = r['Ward No.'] || no;
        return r;
      }));
    } catch (_) {}
  });
  if (data && data.query) rows = searchArray(rows, data.query, Number(data.limit || 100));
  return {success:true, rows:rows.slice(0, Number((data && data.limit) || 100))};
}
...
function wardHeaders() {
  return ['Ward No.','C.N','Name','Address','Family Members','Male','Female','Voters','Phone','Email','Whatsapp',
