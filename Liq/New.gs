...
function getMonthlySummary(month){
  // IMPORTANT: Do not build a month-sized Daily Status table.
  // Calculate the month in memory from the source sheets and save only the
  // compact per-employee result to Monthly Summary.
  ...
  // Include today's ENTRY/OUT/Leave/Holiday in the current-month report.
  // However, today's no-show is NOT marked Absent yet because the 24-hour
  // absence window has not completed. For previous days, no presence means
  // Absent. This also makes a current-day ENTRY count as Working Day even if
  // the employee has not scanned OUT yet.
  let end=isCurrentMonth ? new Date(today.getFullYear(),today.getMonth(),today.getDate()) : new Date(monthLast);
  if(end<first){
    const empty=[]; writeMonthlyRows_(empty); return {success:true,month,rows:empty};
  }
  const finalizedBeforeDate=isCurrentMonth
    ? Utilities.formatDate(new Date(today.getTime()-24*60*60*1000),tz,'yyyy-MM-dd')
  ...
  emps.forEach(e=>{
    let working=0,leave=0,holiday=0,absent=0,workedHours=0,leaveHours=0,extra=0;
    ...
    for(let d=new Date(first);d<=end;d.setDate(d.getDate()+1)){
      ...
      // Keep the same priority as the existing system: Government Holiday,
      // then approved Leave, then Present/Absent.
      if(h){
        holiday++;
        workedHours+=worked;
        if(worked>0) extra+=STANDARD_DAILY_HOURS;
      }else if(l){
        leave++;
        leaveHours+=Number(l.hours||STANDARD_DAILY_HOURS);
      }else if(worked>0 || logs.some(x=>x.type==='ENTRY' || x.type==='OUT')){
        // An attendance scan is a Working Day even when ENTRY exists but OUT
        // has not been recorded yet; in that case Working Hours may be 0.
        working++;
        workedHours+=worked;
      }else if(date<=finalizedBeforeDate){
        // Only dates whose 24-hour window has completed can become Absent.
        absent++;
      }
      // Current-day no-show is intentionally not counted as Absent yet.
    }

    const total=workedHours+leaveHours+extra;
    rows.push({
      month,employeeId:e.id,name:e.name,company:e.company,
      ...
      holidayExtraHours:round2_(extra),totalHours:round2_(total),email:e.email||''
    });
  });
...
function createEmployeeTimeSheetPdf_(employeeId, month, employeeName){
  ...
  for(let d=new Date(first);d<=last;d.setDate(d.getDate()+1)){
    ...
    if(h){
      status=worked>0?'Holiday Worked':'Holiday';
      hours=worked;
      holidayDays++;
      workingHours+=worked;
      if(worked>0){extra=STANDARD_DAILY_HOURS;holidayExtraHours+=extra;}
      note=h.name||'Government Holiday';
    }else if(sunday){
      status=worked>0?'Sunday Worked':'Sunday';
      hours=worked;
      workingHours+=worked;
      if(worked>0){extra=STANDARD_DAILY_HOURS;holidayExtraHours+=extra;}
      note='Weekly Holiday';
      ...
      status='Present';
      hours=worked;
      workingDays++;workingHours+=worked;
    }else{
      status='Absent';
      absentDays++;
    }
  ...
  const html='<!DOCTYPE html><html><head><meta charset="UTF-8"><style>'+
    ...
    '<p style="margin-top:14px">Leave = 8 hours by default. Sunday and government-holiday work = 8 extra hours.</p>'+
    '</body></html>';
