...
function getLocationDirectory(){
  ...
  return {success:true,locations:values.filter(r=>r[0]).map(r=>({id:r[0],name:r[1],category:r[2],phone:r[3],status:r[4]}))};
}
function locationDirectorySheet_(){
  ...
  if(!sh){sh=ss.insertSheet(LOCATION_DIRECTORY_SHEET);sh.appendRow(['Employee ID','Name','Category','Phone','Status']);}
  return sh;
}
function saveLocationDirectory(data){
  data=data||{};const id=String(data.id||data.employeeId||'').trim(),name=String(data.name||'').trim();
  if(!id||!name)return {success:false,error:'Employee ID and Name are required.'};
  const sh=locationDirectorySheet_(),last=sh.getLastRow(),vals=last>=2?sh.getRange(2,1,last-1,5).getDisplayValues():[];
  const row=[id,name,String(data.category||data.trade||'').trim(),String(data.phone||'').trim(),String(data.status||'Active').trim()];
  for(let i=0;i<vals.length;i++)if(String(vals[i][0]).trim()===id){sh.getRange(i+2,1,1,5).setValues([row]);return {success:true,action:'updated'};}
  sh.appendRow(row);return {success:true,action:'created'};
}
