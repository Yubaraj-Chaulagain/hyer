...
function getMonthlySummary(month){
  // SINGLE SOURCE OF TRUTH for Employee Monthly Summary, Company Monthly Summary,
  // Gmail and PDF. The same day rules are used everywhere.
  const parts=String(month||'').split('-').map(Number);
  if(parts.length!==2 || !parts[0] || !parts[1]) return {success:false,error:'Invalid month. Use YYYY-MM.'};

  const year=parts[0], mon=parts[1];
  const tz=Session.getScriptTimeZone();
  const first=new Date(year,mon-1,1);
  const monthLast=new Date(year,mon,0);
  const now=new Date();
  const todayKey=Utilities.formatDate(now,tz,'yyyy-MM-dd');
  const currentMonthKey=Utilities.formatDate(now,tz,'yyyy-MM');
  const isCurrentMonth=String(month)===currentMonthKey;

  // Historical month: use the whole month. Current month: use only dates
  // up to today. Future dates are NEVER Absent and NEVER counted as Holiday.
  ...
  emps.forEach(e=>{
    ...
    for(let d=new Date(first);d<=end;d.setDate(d.getDate()+1)){
      ...
      const pair=hasEntryOutPair_(logs);
      const worked=pair ? calculateWorkedHours_(logs) : 0;

      // Rule order: Sunday -> Government Holiday -> Leave -> Working/Absent.
      // Sunday is a weekly holiday and is not an Absent/Working Day.
      if(sunday){
        if(pair){ workedHours+=worked; extra+=STANDARD_DAILY_HOURS; }
        continue;
      }
      ...
      if(pair){
        ...
        // For the current date, do not mark Absent until its 24-hour window
        // has completed. For historical dates, incomplete/no attendance = Absent.
        if(!isCurrentMonth || date<todayKey) absent++;
      }
    }
    ...
    rows.push({
      ...
      workingDays:working,leaveDays:leave,holidayDays:holiday,absentDays:absent,
      workingHours:round2_(workedHours),leaveHours:round2_(leaveHours),
      holidayExtraHours:round2_(extra),totalHours:round2_(total)
    });
  });

  writeMonthlyRows_(rows);
  return {success:true,month,rows};
}

function hasEntryOutPair_(logs){
  const entries=(logs||[]).filter(x=>x.type==='ENTRY').map(x=>timeMinutes_(x.time)).filter(x=>x>=0).sort((a,b)=>a-b);
  const outs=(logs||[]).filter(x=>x.type==='OUT').map(x=>timeMinutes_(x.time)).filter(x=>x>=0).sort((a,b)=>a-b);
  let oi=0;
...
function getCompanyMonthlySummary(month){
  // Company totals are the sum of employee-day results from getMonthlySummary().
  // Therefore Company Monthly and Employee Monthly can never use different rules.
  const r=getMonthlySummary(month);
  if(!r.success) return r;

  const map={};
  r.rows.forEach(x=>{
    const c=String(x.company||'').trim()||'No Company';
    if(!map[c]) map[c]={month,company:c,employees:0,workingDays:0,leaveDays:0,holidayDays:0,absentDays:0,workingHours:0,leaveHours:0,holidayExtraHours:0,totalHours:0};
    const o=map[c];
    o.employees++;
    o.workingDays+=Number(x.workingDays||0);
    o.leaveDays+=Number(x.leaveDays||0);
    o.holidayDays+=Number(x.holidayDays||0);
    o.absentDays+=Number(x.absentDays||0);
    o.workingHours+=Number(x.workingHours||0);
    o.leaveHours+=Number(x.leaveHours||0);
    o.holidayExtraHours+=Number(x.holidayExtraHours||0);
    o.totalHours+=Number(x.totalHours||0);
  });

  const rows=Object.values(map).sort((a,b)=>String(a.company).localeCompare(String(b.company))).map(x=>{
    x.workingHours=round2_(x.workingHours);
    x.leaveHours=round2_(x.leaveHours);
    x.holidayExtraHours=round2_(x.holidayExtraHours);
    x.totalHours=round2_(x.totalHours);
    return x;
  });
  writeCompanyRows_(rows);
  return {success:true,month,rows};
}
...
function createEmployeeTimeSheetPdf_(employeeId, month, employeeName){
  ...
  const currentMonthKey=Utilities.formatDate(now,tz,'yyyy-MM');
  const isCurrentMonth=String(month)===currentMonthKey;
  const end=isCurrentMonth ? new Date(now.getFullYear(),now.getMonth(),now.getDate()) : monthLast;

  const attendanceMap=buildAttendanceMap_(first,end);
  const leaveMap=buildApprovedLeaveMap_(first,end);
  const holidayMap=buildGovernmentHolidayMap_(first,end);
  const emp=getEmployees().find(e=>String(e.id).toLowerCase()===String(employeeId).toLowerCase());
  const join=String(emp&&emp.joiningDate||'');
  ...
  for(let d=new Date(first);d<=end;d.setDate(d.getDate()+1)){
    ...
    if(sunday){
      status=pair?'Sunday Worked':'Sunday';
      hours=worked;
      if(pair){workingHours+=worked;extra=STANDARD_DAILY_HOURS;holidayExtraHours+=extra;}
      note='Weekly Holiday';
    }else if(h){
      status=pair?'Holiday Worked':'Holiday';
      hours=worked;
      holidayDays++;
      if(pair){workingHours+=worked;extra=STANDARD_DAILY_HOURS;holidayExtraHours+=extra;}
      ...
      status='Leave';
      const lh=Number(l.hours||STANDARD_DAILY_HOURS);
      leaveDays++;leaveHours+=lh;
      note=l.leaveType||'Approved Leave';
    }else if(pair){
      status='Present';
      hours=worked;workingDays++;workingHours+=worked;
    }else if(!isCurrentMonth || date<todayKey){
      status='Absent';absentDays++;
    }else{
      status='Open';note='Current day — 24-hour window not complete';
    }
  ...
  const totalHours=round2_(workingHours+leaveHours+holidayExtraHours);
  const html='<!DOCTYPE html><html><head><meta charset="UTF-8"><style>'+
    ...
    '<table class="summary"><tr><td>Working Days</td><td>'+workingDays+'</td></tr>'+
    '<tr><td>Leave Days</td><td>'+leaveDays+'</td></tr>'+
    '<tr><td>Government Holiday Days</td><td>'+holidayDays+'</td></tr>'+
    '<tr><td>Absent Days</td><td>'+absentDays+'</td></tr>'+
    '<tr><td>Working Hours</td><td>'+round2_(workingHours)+'</td></tr>'+
    '<tr><td>Leave Hours</td><td>'+round2_(leaveHours)+'</td></tr>'+
    '<tr><td>Holiday Extra Hours</td><td>'+round2_(holidayExtraHours)+'</td></tr>'+
    '<tr><td>Total Hours</td><td><b>'+totalHours+'</b></td></tr></table>'+
    ...
    '<p style="margin-top:14px">Leave = 8 hours by default. Sunday and government-holiday work = 8 extra hours. Future dates are not counted.</p>'+
    '</body></html>';

  return HtmlService.createHtmlOutput(html).getBlob().setName(employeeId+'_'+month+'_Time_Sheet.pdf').getAs(MimeType.PDF);
}
...
function setupAttendanceSystem(){
  ...
  return {success:true,message:'System setup complete. Daily Status keeps only the latest finalized day; Monthly Summary stores the monthly history.',triggers:triggers};
}
