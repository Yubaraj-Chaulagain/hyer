...
function doGet(e) {
  try {
    ...
    else if (action === 'setPublicLocation') result = setPublicLocation(parsePayload(p.data));
    else if (action === 'getPublicLocation') result = getPublicLocation(parsePayload(p.data));
    else if (action === 'clearPublicLocation') result = clearPublicLocation(parsePayload(p.data));
    else if (action === 'getLocationDirectory') result = getLocationDirectory();
    else if (action === 'saveLocationDirectory') result = saveLocationDirectory(parsePayload(p.data));
    else if (action === 'deleteLocationDirectory') result = deleteLocationDirectory(parsePayload(p.data));
...
function doPost(e) {
  try {
    ...
    else if(action==='setPublicLocation') result=setPublicLocation(d);
    else if(action==='getPublicLocation') result=getPublicLocation(d);
    else if(action==='clearPublicLocation') result=clearPublicLocation(d);
    else if(action==='getLocationDirectory') result=getLocationDirectory();
    else if(action==='saveLocationDirectory') result=saveLocationDirectory(d);
    else if(action==='deleteLocationDirectory') result=deleteLocationDirectory(d);
...
/* PUBLIC LIVE LOCATION - employee-specific GPS rows */
const PUBLIC_LOCATION_SHEET = 'Public Location';
const LOCATION_DIRECTORY_SHEET = 'Location Directory';
function publicLocationSheet_(){
  const ss=SpreadsheetApp.getActiveSpreadsheet();
  let sh=ss.getSheetByName(PUBLIC_LOCATION_SHEET);
  if(!sh){sh=ss.insertSheet(PUBLIC_LOCATION_SHEET);sh.appendRow(['Employee ID','Employee Name','Updated At','Latitude','Longitude','Accuracy Meters','Active']);}
  // Upgrade legacy 5-column sheet without deleting its existing row.
  const headers=sh.getRange(1,1,1,Math.max(7,sh.getLastColumn())).getDisplayValues()[0];
  if(String(headers[0]).trim()!=='Employee ID'){
    const old=sh.getLastRow()>=2?sh.getRange(2,1,1,Math.min(5,sh.getLastColumn())).getValues()[0]:[];
    sh.clearContents();
    sh.appendRow(['Employee ID','Employee Name','Updated At','Latitude','Longitude','Accuracy Meters','Active']);
    if(old.length && old[1]!=='' && old[2]!=='') sh.appendRow(['LEGACY','Public Location',old[0],old[1],old[2],old[3],old[4]]);
  }
  return sh;
}
function locationId_(data){return String((data&& (data.employeeId||data.id||data.staffId))||'').trim();}
function setPublicLocation(data){
  data=data||{};
  const id=locationId_(data), name=String(data.employeeName||data.name||'').trim();
  const lat=Number(data.latitude),lng=Number(data.longitude),accuracy=Number(data.accuracy||0);
  if(!id)return {success:false,error:'Google Sheet Employee ID is required to share location.'};
  if(!isFinite(lat)||!isFinite(lng)||lat < -90||lat>90||lng < -180||lng>180) return {success:false,error:'Valid GPS latitude and longitude are required.'};
  const sh=publicLocationSheet_(),last=sh.getLastRow();
  const vals=last>=2?sh.getRange(2,1,last-1,7).getDisplayValues():[];
  let rowIndex=-1; for(let i=0;i<vals.length;i++){if(String(vals[i][0]).trim()===id){rowIndex=i+2;break;}}
  const row=[id,name,new Date(),lat,lng,accuracy,true];
  if(rowIndex>0)sh.getRange(rowIndex,1,1,7).setValues([row]);else sh.appendRow(row);
  SpreadsheetApp.flush();
  return {success:true,message:'Public location updated.',employeeId:id,employeeName:name,latitude:lat,longitude:lng,accuracy:accuracy};
}
function getPublicLocation(data){
  data=data||{}; const id=locationId_(data); const sh=publicLocationSheet_(),last=sh.getLastRow();
  if(last<2)return {success:true,location:null};
  const vals=sh.getRange(2,1,last-1,7).getValues();
  let r=null;
  if(id){for(let i=0;i<vals.length;i++)if(String(vals[i][0]).trim()===id){r=vals[i];break;}}
  else {for(let i=vals.length-1;i>=0;i--)if(String(vals[i][6]).toLowerCase()==='true'){r=vals[i];break;}}
  if(!r)return {success:true,location:null};
  const active=String(r[6]).toLowerCase()==='true';
  return {success:true,location:{employeeId:String(r[0]||''),employeeName:String(r[1]||''),updatedAt:r[2] instanceof Date?Utilities.formatDate(r[2],Session.getScriptTimeZone(),'yyyy-MM-dd HH:mm:ss'):String(r[2]||''),latitude:Number(r[3]),longitude:Number(r[4]),accuracy:Number(r[5]||0),active:active}};
}
function clearPublicLocation(data){
  data=data||{}; const id=locationId_(data); if(!id)return {success:false,error:'Employee ID is required to stop sharing.'};
  const sh=publicLocationSheet_(),last=sh.getLastRow();
  if(last>=2){const vals=sh.getRange(2,1,last-1,7).getDisplayValues();for(let i=0;i<vals.length;i++)if(String(vals[i][0]).trim()===id){sh.getRange(i+2,3,1,5).setValues([[new Date(),'','','',false]]);SpreadsheetApp.flush();return {success:true,message:'Location sharing stopped.',employeeId:id};}}
  return {success:true,message:'No active location row found.',employeeId:id};
}
function getLocationDirectory(){
  const sh=locationDirectorySheet_(),last=sh.getLastRow();
  if(last<2)return {success:true,locations:[]};
  // A:H = Employee ID, Name, Category, Phone, Status, Location Name, Latitude, Longitude
  const values=sh.getRange(2,1,last-1,8).getDisplayValues();
  return {success:true,locations:values.filter(r=>r[0]).map(r=>({
    id:String(r[0]||''),employeeId:String(r[0]||''),name:String(r[1]||''),employeeName:String(r[1]||''),
    category:String(r[2]||''),phone:String(r[3]||''),status:String(r[4]||'Active'),
    locationName:String(r[5]||''),latitude:r[6]===''?'':Number(r[6]),longitude:r[7]===''?'':Number(r[7])
  }))};
}
function locationDirectorySheet_(){
  const ss=SpreadsheetApp.getActiveSpreadsheet();let sh=ss.getSheetByName(LOCATION_DIRECTORY_SHEET);
  const headers=['Employee ID','Name','Category','Phone','Status','Location Name','Latitude','Longitude'];
  if(!sh){sh=ss.insertSheet(LOCATION_DIRECTORY_SHEET);sh.appendRow(headers);return sh;}
  // Safely upgrade the old 5-column directory without deleting existing records.
  if(sh.getMaxColumns()<8)sh.insertColumnsAfter(sh.getMaxColumns(),8-sh.getMaxColumns());
  sh.getRange(1,1,1,8).setValues([headers]);
  return sh;
}
function saveLocationDirectory(data){
  data=data||{};
  const id=String(data.id||data.employeeId||'').trim(),name=String(data.name||data.employeeName||'').trim();
  const locationName=String(data.locationName||data.location||data.address||'').trim();
  const latRaw=data.latitude!==undefined?data.latitude:data.lat;
  const lngRaw=data.longitude!==undefined?data.longitude:(data.lng!==undefined?data.lng:data.lon);
  const lat=Number(latRaw),lng=Number(lngRaw);
  if(!id||!name)return {success:false,error:'Employee ID and Name are required.'};
  if(!locationName)return {success:false,error:'Location Name is required.'};
  if(latRaw===''||lngRaw===''||latRaw===null||lngRaw===null||!isFinite(lat)||!isFinite(lng)||lat < -90||lat>90||lng < -180||lng>180)
    return {success:false,error:'Valid Latitude and Longitude are required.'};
  const sh=locationDirectorySheet_(),last=sh.getLastRow(),vals=last>=2?sh.getRange(2,1,last-1,1).getDisplayValues():[];
  const row=[id,name,String(data.category||data.trade||'').trim(),String(data.phone||'').trim(),String(data.status||'Active').trim(),locationName,lat,lng];
  for(let i=0;i<vals.length;i++)if(String(vals[i][0]).trim()===id){sh.getRange(i+2,1,1,8).setValues([row]);SpreadsheetApp.flush();return {success:true,action:'updated',message:'Location updated with coordinates.'};}
  sh.appendRow(row);SpreadsheetApp.flush();return {success:true,action:'created',message:'Location saved with coordinates.'};
}
function deleteLocationDirectory(data){
  const id=String((data&& (data.id||data.employeeId))||'').trim();if(!id)return {success:false,error:'Employee ID is required.'};
  const sh=locationDirectorySheet_(),last=sh.getLastRow();if(last<2)return {success:true,deleted:false};
  const vals=sh.getRange(2,1,last-1,1).getDisplayValues();for(let i=vals.length-1;i>=0;i--)if(String(vals[i][0]).trim()===id){sh.deleteRow(i+2);return {success:true,deleted:true};}
