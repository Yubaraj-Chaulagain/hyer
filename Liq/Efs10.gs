...
function doGet(e) {
  try {
    ...
    else if (action === 'getPublicLocation') result = getPublicLocation(parsePayload(p.data).employeeId||'');
    else if (action === 'getPublicLocations') result = getPublicLocations();
    else if (action === 'clearPublicLocation') result = clearPublicLocation(parsePayload(p.data).employeeId||'');
...
function doPost(e) {
  try {
    ...
    else if(action==='getPublicLocation') result=getPublicLocation(d.employeeId||'');
    else if(action==='getPublicLocations') result=getPublicLocations();
    else if(action==='clearPublicLocation') result=clearPublicLocation(d.employeeId||'');
...
/* PUBLIC LIVE LOCATION - one independently keyed row per Google Sheet Employee ID */
const PUBLIC_LOCATION_SHEET = 'Public Location';
function publicLocationSheet_(){
  const ss=SpreadsheetApp.getActiveSpreadsheet();let sh=ss.getSheetByName(PUBLIC_LOCATION_SHEET);
  const headers=['Employee ID','Name','Trade','Company','Phone','Updated At','Latitude','Longitude','Accuracy Meters','Active'];
  if(!sh){sh=ss.insertSheet(PUBLIC_LOCATION_SHEET);sh.getRange(1,1,1,headers.length).setValues([headers]);sh.setFrozenRows(1);}
  else if(sh.getLastRow()===0){sh.getRange(1,1,1,headers.length).setValues([headers]);sh.setFrozenRows(1);}
  return sh;
}
function setPublicLocation(data){
  const id=String(data.employeeId||'').trim(),name=String(data.name||'').trim();
  const lat=Number(data.latitude),lng=Number(data.longitude),accuracy=Number(data.accuracy||0);
  if(!id||!name)return {success:false,error:'Select an employee from the Google Sheet.'};
  ...
  const sh=publicLocationSheet_(),row=[id,name,String(data.trade||''),String(data.company||''),String(data.phone||''),new Date(),lat,lng,accuracy,true];
  const last=sh.getLastRow();let target=0;
  if(last>=2){const ids=sh.getRange(2,1,last-1,1).getDisplayValues();for(let i=0;i<ids.length;i++)if(String(ids[i][0]).trim()===id){target=i+2;break;}}
  if(target)sh.getRange(target,1,1,row.length).setValues([row]);else sh.appendRow(row);
  SpreadsheetApp.flush();return {success:true,message:'Live location updated.',employeeId:id,latitude:lat,longitude:lng,accuracy:accuracy};
}
function publicLocationObject_(r){return {employeeId:String(r[0]||''),name:String(r[1]||''),trade:String(r[2]||''),company:String(r[3]||''),phone:String(r[4]||''),updatedAt:r[5] instanceof Date?Utilities.formatDate(r[5],Session.getScriptTimeZone(),'yyyy-MM-dd HH:mm:ss'):String(r[5]||''),latitude:Number(r[6]),longitude:Number(r[7]),accuracy:Number(r[8]||0),active:String(r[9]).toLowerCase()==='true'};}
function getPublicLocations(){
  const sh=publicLocationSheet_();if(sh.getLastRow()<2)return {success:true,locations:[]};
  const rows=sh.getRange(2,1,sh.getLastRow()-1,10).getValues().filter(r=>String(r[0]||'').trim());
  return {success:true,locations:rows.map(publicLocationObject_).filter(x=>x.active)};
}
function getPublicLocation(employeeId){
  const all=getPublicLocations().locations||[];const found=employeeId?all.find(x=>x.employeeId===String(employeeId)):all[0];
  return {success:true,location:found||null};
}
function clearPublicLocation(employeeId){
  const id=String(employeeId||'').trim();if(!id)return {success:false,error:'Employee ID is required to stop sharing.'};
  const sh=publicLocationSheet_(),last=sh.getLastRow();if(last>=2){const ids=sh.getRange(2,1,last-1,1).getDisplayValues();for(let i=0;i<ids.length;i++)if(String(ids[i][0]).trim()===id){const row=i+2;sh.getRange(row,6,1,5).setValues([[new Date(),'','','',false]]);SpreadsheetApp.flush();return {success:true,message:'Location sharing stopped for '+id};}}
  return {success:true,message:'No active location found for '+id};
}
