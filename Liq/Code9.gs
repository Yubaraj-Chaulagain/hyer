...
function doGet(e) {
  try {
    ...
    else if (action === 'getPublicLocation') result = getPublicLocation();
    else if (action === 'clearPublicLocation') result = clearPublicLocation();
    else if (action === 'getLocationDirectory') result = getLocationDirectory_();
    else if (action === 'saveLocationDirectory') result = saveLocationDirectory_(parsePayload(p.data));
    else if (action === 'deleteLocationDirectory') result = deleteLocationDirectory_(parsePayload(p.data).id);
...
function doPost(e) {
  try {
    ...
    else if(action==='saveLocationDirectory') result=saveLocationDirectory_(d);
    else if(action==='deleteLocationDirectory') result=deleteLocationDirectory_(d.id);
...
/* PUBLIC LIVE LOCATION - latest owner GPS shared publicly by explicit action */
...
function publicLocationSheet_(){
  ...
  if(!sh){sh=ss.insertSheet(PUBLIC_LOCATION_SHEET);sh.appendRow(['Updated At','Latitude','Longitude','Accuracy Meters','Active']);}
  return sh;
}
function setPublicLocation(data){
  ...
  if(!isFinite(lat)||!isFinite(lng)||lat < -90||lat>90||lng < -180||lng>180)
    return {success:false,error:'Valid GPS latitude and longitude are required.'};
  const sh=publicLocationSheet_();
  const row=[new Date(),lat,lng,accuracy,true];
  if(sh.getLastRow()>=2) sh.getRange(2,1,1,5).setValues([row]);
  else sh.appendRow(row);
  SpreadsheetApp.flush();
  return {success:true,message:'Public location updated.',latitude:lat,longitude:lng,accuracy:accuracy};
}
function getPublicLocation(){
  const sh=publicLocationSheet_();
  if(sh.getLastRow()<2)return {success:true,location:null};
  const r=sh.getRange(2,1,1,5).getValues()[0];
  const active=String(r[4]).toLowerCase()==='true';
  return {success:true,location:{updatedAt:r[0] instanceof Date?Utilities.formatDate(r[0],Session.getScriptTimeZone(),'yyyy-MM-dd HH:mm:ss'):String(r[0]||''),latitude:Number(r[1]),longitude:Number(r[2]),accuracy:Number(r[3]||0),active:active}};
}
function clearPublicLocation(){
  const sh=publicLocationSheet_();
  if(sh.getLastRow()>=2)sh.getRange(2,1,1,5).setValues([[new Date(),'','','',false]]);
  SpreadsheetApp.flush();
  return {success:true,message:'Public location sharing stopped.'};
}


/* LOCATION DIRECTORY FOR ADMIN LOCATION SETTINGS + PUBLIC FOLLOW ME */
const LOCATION_DIRECTORY_SHEET_ = 'Location Directory';
function locationDirectorySheet_(){
  const ss=SpreadsheetApp.getActive();
  let sh=ss.getSheetByName(LOCATION_DIRECTORY_SHEET_);
  const headers=['ID','Name','Category','Location Name','Latitude','Longitude','Phone','Status','Updated At'];
  if(!sh){sh=ss.insertSheet(LOCATION_DIRECTORY_SHEET_);sh.appendRow(headers);sh.setFrozenRows(1);}
  else if(sh.getLastRow()===0){sh.appendRow(headers);sh.setFrozenRows(1);}
  return sh;
}
function getLocationDirectory_(){
  ...
  const locations=vals.filter(r=>r[0]).map(r=>({
    id:String(r[0]),name:String(r[1]||''),category:String(r[2]||'Other'),
    locationName:String(r[3]||''),latitude:Number(r[4]),longitude:Number(r[5]),
    phone:String(r[6]||''),status:String(r[7]||'Active'),
    updatedAt:r[8] instanceof Date?Utilities.formatDate(r[8],Session.getScriptTimeZone(),'yyyy-MM-dd HH:mm:ss'):String(r[8]||'')
  }));
  return {success:true,locations:locations};
}
function saveLocationDirectory_(d){
  const name=String(d.name||'').trim(),category=String(d.category||'Other').trim();
  const locationName=String(d.locationName||'').trim(),lat=Number(d.latitude),lng=Number(d.longitude);
  if(!name||!locationName||!isFinite(lat)||lat < -90||lat>90||!isFinite(lng)||lng < -180||lng>180)
    return {success:false,error:'Name, Location Name, and valid Latitude/Longitude are required.'};
  const sh=locationDirectorySheet_(),id=String(d.id||Utilities.getUuid());
  const row=[id,name,category,locationName,lat,lng,String(d.phone||''),String(d.status||'Active'),new Date()];
  const last=sh.getLastRow();let target=0;
  if(last>=2){const ids=sh.getRange(2,1,last-1,1).getDisplayValues().flat();const ix=ids.indexOf(id);if(ix>=0)target=ix+2;}
  if(target)sh.getRange(target,1,1,row.length).setValues([row]);else sh.appendRow(row);
  SpreadsheetApp.flush();
  return {success:true,message:'Location saved successfully.',id:id};
}
function deleteLocationDirectory_(id){
  const sh=locationDirectorySheet_(),last=sh.getLastRow();if(last<2)return {success:true};
  const ids=sh.getRange(2,1,last-1,1).getDisplayValues().flat(),ix=ids.indexOf(String(id));
  if(ix<0)return {success:false,error:'Location ID not found.'};
  sh.deleteRow(ix+2);return {success:true,message:'Location deleted.'};
}
